...
**Summary.** Five requests were issued and captured in full: two baseline `200 OK` responses against distinct endpoints, one deliberate `404 Not Found` against a resource that does not exist, one `201 Created` resulting from a POST request, and one `HEAD` request demonstrating header only retrieval. Together these five pairs demonstrate the correspondence between HTTP status codes, the `Content-Type` header, and the presence or absence of a response body.